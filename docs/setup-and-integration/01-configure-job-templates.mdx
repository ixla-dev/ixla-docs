...
Once files were transferred, the system will perform the following actions automatically: 
...
We can generate *Data Exchange Tables* 

- Create a *Data Exchange Table* for the specified job template
- Create a new *Data Source* for this job template. Data Source contain information such as: 
  - Type (For now only relational databases)
  - Connection string 
  - Table Name 
  - Query Template
...
```csharp
  ...
  ...
  _etl.GenerateDataSource(jobTemplateId)
```
