...
:::info
This is a very quick introduction to SAMLight's *Job Editor* the concepts of *Entities*, *Pens* and *SJF files*. 
If you're already familiar with *SCAPS SAMLight* feel free to [skip this section](./configure-job-templates).
:::

Behind the scenes, *AIDA* uses *SAMLight*'s *CCI (Client Control Interface)* to handle the engraving process.
...
Pens can be tweaked to find a balance between performance and printing quaility that matches application's requirements. Ideal parameters selection depends on many factors and is a topic out of the scope of this guide. 
...
Jobs created with SAMLight can be saved as `.sjf` (SCAPS Job Format) files. AIDA manages SJF files and interfaces with SAMLight via CCI interface to handle the engraving process.
