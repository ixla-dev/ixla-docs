...
The IXLA BOX is a modular laser engraving system designed for plastic cards personalization applications with mid-high volume production requirements.
...
Alongside IWS we provide a webapp that can be used to quickly prepare the system and start issuing cards. It is built on top of IWS's REST APIs, meaning that if needed, integrators can build their own tools and applications with the same featureset.

### Features
* Setup Wizard for template Creation and Configuration
    * Import *SJF Files*
    * Create *Layouts* (associate SFJ files to card sides for engraving)
    * Generate *Data Exchange Tables* 
* Configure XY Auto-Positioning parameters
